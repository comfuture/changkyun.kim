...
위 예시의 경우는 페이징을 마치 RDBMS의 `LIMIT n, m` 으로 다루듯 마지막 단계에만 페이징을 위한 스테이지를 추가하는 경우를 보여주는 예시로, 몽고디비에서 쉽게 저지를 수 있는 실수에 해당한다. `$lookup` 스테이지를 직접 `$limit` 스테이지 다음으로 옮겨두면 극적으로 성능 향상을 기대할 수 있다.
